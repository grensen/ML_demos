// https://github.com/grensen/neural_network_pruning

#if DEBUG
    Console.WriteLine("Debug mode is on, switch to Release mode");
#endif

Console.WriteLine($"\nBegin Neural Network One-Shot Compression Pruning Demo\n");

string path = @"C:\compression_net\";
string filePath = path + @"compression_net.txt";
...
var pruning = PruningNetworkMemoryAllocation(net);

PruningWeightInit(net, pruning.positions, pruning.weights, SEED);

var trained = RunTrainingWithPruning(PARALLEL, d, 
    net, pruning.positions, pruning.weights, PRUNING, NODEPROB,
    60000, LEARNINGRATE, MOMENTUM, FACTOR, EPOCHS, BATCHSIZE);

RunTestWithPruning(PARALLEL, d, trained.net, trained.positions, trained.weights, 10000);
...
static (int[] net, int[][] positions, float[][] weights) RunTrainingWithPruning(bool multiCore, AutoData d,
    ...
    int len, float lr, float mom, float FACTOR, int EPOCHS, int BATCHSIZE)
{
    ...
    for (int i = 0; i < weights.Length; i++)
        deltas[i] = new float[weights[i].Length];
...
// 1. memory allocation
static (int[][] positions, float[][] weights) PruningNetworkMemoryAllocation(int[] net)
{
    return (CreateJaggedArray<int>(net), CreateJaggedArray<float>(net));
}
static T[][] CreateJaggedArray<T>(int[] net)
{
    int networkSize = net.Sum();
...
static void PruningWeightInit(int[] net, int[][] positions, float[][] weights, int seed)
{
    ...
    {
        float sd = MathF.Sqrt(6.0f / (net[i] + weights[j].Length));
        for (int l = 0; l < net[i]; l++)
            for (int r = 0; r < weights[j].Length; r++)
            {
                positions[j + l][r] = k + r;
...
static void PruningFeedForward(int[] net, float[] neurons, int[][] positions, float[][] weights)
{
    ...
    {
        for (int l = j; l < j + net[i]; l++)
        {
            var n = neurons[l];
            if (n <= 0) continue;
            for (int R = weights[l].Length, r = 0; r < R; r++)
            {
                int id = positions[l][r];
                neurons[id] = weights[l][r] * n + neurons[id];
            }
        }
        j += net[i];
    }
...
{
    float[] gradients = new float[neurons.Length]; // right output side

    for (int r = neurons.Length - net[^1], p = 0; r < neurons.Length; r++, p++)
        gradients[r] = target == p ? 1 - neurons[r] : -neurons[r];

    for (int j = gradients.Length - net[^1], i = net.Length - 2; i >= 0; i--)
    {
        j -= net[i];
        for (int l = 0; l < net[i]; l++)
        {
...
static int CompressionPruning(ref int[] net, ref int[][] positions, ref float[][] weights, ref float[][] deltas, float pr, float np)
{
    ...
    int networkSize = StructuredPruning(ref net, ref positions, ref weights, ref deltas, np);
    NetworkPruningInfo(net, positions, weights);
...
static void UnstructuredPruning(int[] net, int[][] positions, float[][] weights, float[][] deltas, float pr)
{
    ...
    for (int jl = 0; jl < positions.Length; jl++) // each input to outputs connected neuron (input + hidden)
        for (int r = 0; r < positions[jl].Length; r++) // each connected weight on all outputs
            if (weights[jl][r] * weights[jl][r] < pr) // magnitude weight pruning
            {
...
static int StructuredPruning(ref int[] net, ref int[][] positions, ref float[][] weights, ref float[][] deltas, float id)
{
    // 1. structured pruning based on low magnitude weights count
    bool[] removeNode = new bool[positions.Length];
    for (int i = 0, j = 0, k = net[0]; i < net.Length - 1; i++)
    {
        // input to center node
        for (int l = 0; l < net[i]; l++)
            if (weights[j + l].Length / (float)net[i + 1] < id)
                removeNode[j + l] = true;
    ...
    for (int i = 0, j = 0, k = net[0]; i < net.Length - 2; i++) // from first to last hidden layer
    {
        ...
        for (int r = 0; r < net[i + 1]; r++) // each hidden neuron output side
        {
            ...
            {
                // 2.2 remove row as outgoing weights: weights, positions, deltas                    
                ...
                // 2.3 remove rowingoing connections col: weights, positions, deltas
                for (int jl = j; jl < positions[j + r].Length; jl++)
                {
                    int last = positions[jl].Length - 1;
                    for (int rr = r; rr < last; rr++)
                    {
                        positions[jl][rr] = positions[jl][rr + 1];
                        weights[jl][rr] = weights[jl][rr + 1];
                        deltas[jl][rr] = deltas[jl][rr + 1];
                ...
                for (int jl = j; jl < positions.Length; jl++)
...
void PrintHyperparameters()
{
    ...
    Console.WriteLine("POSITIONS    = " + net.Zip(net.Skip(1), (prev, curr) => prev * curr).Sum());
