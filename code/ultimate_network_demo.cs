...
string path = @"C:\ultimate_net\";
string filePath = path + @"ultimate_net.txt";

AutoData d = new(path); // get data

// define ultimate network 
int[] net = { 784, 20,20,20, 20, 20, 10 }; 
...
SaveUltimateNetToFile(filePath, trained.net, trained.weights);

Console.WriteLine($"Load {networkType} network from: " + filePath);
var net2 = LoadUltimateNetFromFile(filePath);

RunUltimateTest(PARALLEL, d, net2.net, net2.weights, 10000);
NetworkInfo(net2.net, net2.weights);
            ...
            if (multiCore)
                Parallel.For(b * BATCHSIZE, (b + 1) * BATCHSIZE, x =>
                {
                    (c[indices[x]], ce[indices[x]]) = Train(
                        ...
                        net, weights, deltas, networkSize);
                });
            else
                for (int x = b * BATCHSIZE, X = (b + 1) * BATCHSIZE; x < X; x++)
                    (c[indices[x]], ce[indices[x]]) = Train(d.samplesTrainingF.AsSpan().Slice(indices[x] * inputSize, inputSize),
                        d.labelsTraining[indices[x]], net, weights, deltas, networkSize);
...
static (bool, float) Train(Span<float> sample, int target,
    int[] net, float[][] weights, float[][] deltas, int networkSize)
{
    // ff
...
// 3. feed forward
static void FeedForward(int[] net, float[] neurons, float[][] weights)
{
    int j = 0, k = net[0];
...
static void Backprop2(int[] net, float[] neurons, float[][] weight, float[][] deltas, int target)
{
    float[] gradients = new float[neurons.Length]; // all gradients
...
static void SaveUltimateNetToFile(string fileName, int[] net, float[][] weights)
{
...
static (int[] net, float[][] weights) LoadUltimateNetFromFile(string fileName)
{
    ...
    float[][] weights = new float[lines.Length - 1][];
...
static void NetworkInfo(int[] net, float[][] weights)
{
    Console.WriteLine($"Network = {string.Join("-", net)} = {net.Sum()} (Weights: {GetWeightsSize(weights)})\n");
}
